<template>
   <el-header>
     <el-menu
      ...
      <el-menu-item index='3'  @click="jumpPage({path:'/dataset'})">
        数据集
      </el-menu-item>
      <el-menu-item index='4'  @click="jumpPage({path:'/innovation'})">
        创新平台
      </el-menu-item>
      <el-menu-item index='5' @click="jumpPage({path:'/nlpschool'})">
        NLP学院
      </el-menu-item>
      {{token}}
...
import { getToken, getUserName } from '@/utils/auth'
  export default {
    ...
    methods: {
      ...
      jumpPage(params){
        this.activeIndex = "0"
        if(params.path == "/innovation"|| params.path == "/nlpschool"){
      ...
      linkServiceListPage (id) {
        ...
        // document.querySelectorAll(".el-menu")[1].style.display="none"     
        // this.$router.push({path: '/serviceLists/'+Math.random().toString(36).substr(2) })
        window.location.href =window.location.origin+ "?name=serviceLists"
      },
      // 跳转到解决方案详情页面
      linkSolutionPage(item){
        ...
        if(item.forwardType == '011002'){
          this.$router.push({path:"/solution/"+item.solutionUrl,params:{id:item.id}})
        }else{
